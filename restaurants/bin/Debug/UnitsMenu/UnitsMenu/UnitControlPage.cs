...
namespace UnitMenu
{
    public class UnitControlPage
    {
        public static void GetUnits(int userId)
        {
            ...
            Form form = new Form
            {
                Text = "Управление единицами измерения",
                Width = 600,
                Height = 400
            };
            ...
            addButton.Click += (addSender, addArgs) =>
            {
                ShowEditForm(null, (name) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string insertQuery = "INSERT INTO Measurement_units (Name) VALUES (@Name)";
            ...
            editButton.Click += (editSender, editArgs) =>
            {
                ...
                ShowEditForm(currentName, (newName) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string updateQuery = "UPDATE Measurement_units SET Name = @Name WHERE Id = @Id";
            ...
            deleteButton.Click += (deleteSender, deleteArgs) =>
            {
                ...
                DataGridViewRow selectedRow = dataGridView.SelectedRows[0];
                string unitName = selectedRow.Cells["Название"].Value?.ToString();
                string unitId = selectedRow.Cells["Id"].Value?.ToString();

                DialogResult result = MessageBox.Show($"Вы уверены, что хотите удалить '{unitName}'?", "Подтверждение удаления", MessageBoxButtons.YesNo);
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string update1Query = "UPDATE Stocks SET Measurement_unit_id = 0 WHERE Measurement_unit_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(update1Query, connection))
                            {
                                command.Parameters.AddWithValue("@Id", unitId);
                                command.ExecuteNonQuery();
                            }
                            string update2Query = "UPDATE Requests_details SET Measurement_unit_id = 0 WHERE Measurement_unit_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(update2Query, connection))
                            {
                                command.Parameters.AddWithValue("@Id", unitId);
                                command.ExecuteNonQuery();
                            }
                            string update3Query = "UPDATE Receipts_details SET Measurement_unit_id = 0 WHERE Measurement_unit_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(update3Query, connection))
                            {
                                command.Parameters.AddWithValue("@Id", unitId);
                                command.ExecuteNonQuery();
                            }
                            string deleteQuery = "DELETE FROM Measurement_units WHERE Id = @Id";
        ...
        private static DataTable LoadData()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT Id, Name AS Название FROM Measurement_units WHERE Id > 0";
        ...
        private static (int Add, int Edit, int Delete) GetUserPermissions(int userId)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@MenuId", 8);
