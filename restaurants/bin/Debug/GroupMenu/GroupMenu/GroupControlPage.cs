...
namespace GroupMenu
{
    public class GroupControlPage
    {
        public static void GetGroups(int userId)
        {
            ...
            addButton.Click += (addSender, addArgs) =>
            {
                ShowEditForm(null, (name) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string insertQuery = "INSERT INTO Groups (Name) VALUES (@Name)";
            ...
            editButton.Click += (editSender, editArgs) =>
            {
                ...
                ShowEditForm(currentName, (newName) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string updateQuery = "UPDATE Groups SET Name = @Name WHERE Id = @Id";
            ...
            deleteButton.Click += (deleteSender, deleteArgs) =>
            {
                ...
                DataGridViewRow selectedRow = dataGridView.SelectedRows[0];
                string groupName = selectedRow.Cells["Название"].Value?.ToString();
                string groupId = selectedRow.Cells["Id"].Value?.ToString();

                DialogResult result = MessageBox.Show($"Вы уверены, что хотите удалить '{groupName}'?", "Подтверждение удаления", MessageBoxButtons.YesNo);
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        using (SQLiteConnection connection = new SQLiteConnection(connectionString))
                        {
                            ...
                            string updateQuery = "UPDATE Assortment SET Group_id = 0 WHERE Group_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(updateQuery, connection))
                            {
        ...
        private static DataTable LoadData()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT Id, Name AS Название FROM Groups WHERE Id > 0";
        ...
        private static (int Add, int Edit, int Delete) GetUserPermissions(int userId)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@MenuId", 14);
