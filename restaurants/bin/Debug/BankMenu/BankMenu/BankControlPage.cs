    ...
    public class BankControlPage
    {
        public static void GetBanks(int userId)
        {
            ...
            addButton.Click += (addSender, addArgs) =>
            {
                ShowEditForm(null, (name) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string insertQuery = "INSERT INTO Banks (Name) VALUES (@Name)";
            ...
            editButton.Click += (editSender, editArgs) =>
            {
                ...
                ShowEditForm(currentName, (newName) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string updateQuery = "UPDATE Banks SET Name = @Name WHERE Id = @Id";
            ...
            deleteButton.Click += (deleteSender, deleteArgs) =>
            {
                ...
                DataGridViewRow selectedRow = dataGridView.SelectedRows[0];
                string bankName = selectedRow.Cells["Название"].Value?.ToString();
                string bankId = selectedRow.Cells["Id"].Value?.ToString();

                DialogResult result = MessageBox.Show($"Вы уверены, что хотите удалить '{bankName}'?", "Подтверждение удаления", MessageBoxButtons.YesNo);
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string updateQuery = "UPDATE Suppliers SET Bank_id = 0 WHERE Bank_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(updateQuery, connection))
                            {
                                command.Parameters.AddWithValue("@Id", bankId);
                                command.ExecuteNonQuery();
                            }
                            string deleteQuery = "DELETE FROM Banks WHERE Id = @Id";
        ...
        private static DataTable LoadData()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT Id, Name AS Название FROM Banks WHERE Id > 0";
        ...
        private static (int Add, int Edit, int Delete) GetUserPermissions(int userId)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@MenuId", 13);
