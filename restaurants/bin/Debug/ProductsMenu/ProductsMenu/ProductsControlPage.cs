...
{
    public class ProductsControlPage
    {
        public static void GetProducts(int userId)
        {
            ...
            addButton.Click += (addSender, addArgs) =>
            {
                ShowEditForm(null, (name) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string insertQuery = "INSERT INTO Products (Name) VALUES (@Name)";
            ...
            editButton.Click += (editSender, editArgs) =>
            {
                ...
                ShowEditForm(currentName, (newName) =>
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string updateQuery = "UPDATE Products SET Name = @Name WHERE Id = @Id";
            ...
            deleteButton.Click += (deleteSender, deleteArgs) =>
            {
                ...
                DialogResult result = MessageBox.Show($"Вы уверены, что хотите удалить '{productName}'?", "Подтверждение удаления", MessageBoxButtons.YesNo);
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        {
                            connection.Open();
                            string update1Query = "UPDATE Stocks SET Product_id = 0 WHERE Product_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(update1Query, connection))
                            {
                                command.Parameters.AddWithValue("@Id", productId);
                                command.ExecuteNonQuery();
                            }
                            string updat2eQuery = "UPDATE Receipts_details SET Product_id = 0 WHERE Product_id = @Id";
                            using (SQLiteCommand command = new SQLiteCommand(updat2eQuery, connection))
                            {
                                command.Parameters.AddWithValue("@Id", productId);
                                command.ExecuteNonQuery();
                            }
        ...
        private static DataTable LoadData()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT Id, Name AS Название FROM Products WHERE Id > 0";
        ...
        private static (int Add, int Edit, int Delete) GetUserPermissions(int userId)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@MenuId", 12);
